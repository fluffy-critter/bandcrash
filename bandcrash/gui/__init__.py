...
class PreferencesWindow(QDialog):
    ...
    def connect_butler(self):
        ...
        connection = subprocess.run(
            [self.butler_path.text(), "login", "--assume-yes"],
            capture_output=True,
            check=False,
    ...
    def show_preferences():
        ...
        if instance := BandcrashApplication.bc_instance():
...
class AlbumEditor(QMainWindow):
    ...
    class PathDelegate:
        ...
        def get_last_directory(
            self, role: FileRole, file_path: typing.Optional[str] = None
        ):
            ...
            if file_path:
                ...
                if self.filename:
                    ...
                    return os.path.dirname(
                        util.make_absolute_path(self.filename)(file_path)
                    )
            ...
            if self.filename:
                ...
                if role.name in self.last_directory:
                    ...
                    return util.make_absolute_path(self.filename)(
                        self.last_directory[role.name]
                    )
        ...
        def set_last_directory(self, role: FileRole, dir_path: str):
            ...
            if self.filename:
                self.last_directory[role.name] = util.make_relative_path(self.filename)(
                    dir_path
                )
            else:
    ...
    def __init__(self, path: str):
        ...
        add_menu_item(
            file_menu, "&New", BandcrashApplication.file_new, standard_key.New
        )
        add_menu_item(
            file_menu, "&Open...", BandcrashApplication.file_open, standard_key.Open
        )
        ...
        add_menu_item(
            edit_menu,
            "&Preferences",
            PreferencesWindow.show_preferences,
        ...
        add_menu_item(
            track_menu, "&Add...", self.track_listing.add_track_button, "Ctrl+Shift+A"
        )
        add_menu_item(
            track_menu, "&Delete", self.track_listing.delete_track, "Ctrl+Del"
        )
        add_menu_item(
            track_menu, "&Previous", self.track_listing.select_previous, "PgUp"
        )
        ...
        add_menu_item(
            track_menu, "Move &down", self.track_listing.move_down, "Alt+Down"
        )
        ...
        for widget in (self.artist, self.title, self.genre, self.composer):
    ...
    def apply(self):
        ...
        datatypes.apply_text_fields(
            self.data, (("artwork", self.artwork.file_path),), relpath
        )

        datatypes.apply_text_fields(self.data, (("year", self.year),), int)

        datatypes.apply_checkbox_fields(
            self.data,
            (
        ...
        datatypes.apply_text_fields(
            theme, (("user_css", self.user_css.file_path),), relpath
        )
        datatypes.apply_checkbox_fields(
            theme, (("hide_footer", self.hide_footer, False),)
        )
        ...
        self.data["_gui"] = {"lastdir": self.path_delegate.last_directory}
    ...
    def undo_step(self):
        ...
        if self.undo_history:
            ...
            LOGGER.debug(
                "history size = %d/%d", len(
                    self.undo_history), len(self.redo_history)
            )
    ...
    def encode_album(self):
        ...
        filename_parts = [
            part
            for part in [self.data.get(field) for field in ("artist", "title")]
            if part
        ]
        ...
        config.butler_args = (
            BandcrashApplication.bc_instance().config.butler_args.split()
        )
    ...
    def renormalize_paths(self, old_name, new_name):
        ...
        self.path_delegate.last_directory.update(
            {
                key: renorm(value)
                for key, value in self.path_delegate.last_directory.items()
            }
        )
    ...
    def closeEvent(self, event):
        ...
        if do_close:
            BandcrashApplication.bc_instance().release_editor(self)
        else:
            event.ignore()
    ...
    def open_manual():
        ...
        QtGui.QDesktopServices.openUrl(
            QtCore.QUrl("https://bandcrash.readthedocs.io"))
...
class BandcrashApplication(QApplication):
    ...
    def file_new():
        ...
        BandcrashApplication.bc_instance().open_file("")
    ...
    def file_open(or_new: bool = False):
        ...
        path, _ = QFileDialog.getOpenFileName(
            None, "Open album", role.default_directory, role.file_filter
        )
        if path or or_new:
            if path:
                role.default_directory = os.path.dirname(path)
            BandcrashApplication.bc_instance().open_file(path)
    ...
    def event(self, evt, *args, **kws):
        ...
        return super().event(evt, *args, **kws)
...
def main():
    ...
    logging.basicConfig(
        level=LOG_LEVELS[min(options.verbosity, len(LOG_LEVELS) - 1)],
        format="%(message)s",
    )

    LOGGER.debug("Opening bandcrash GUI with provided files: %s",
                 options.open_files)
